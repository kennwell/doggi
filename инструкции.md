...
4. **(Опционально) IPinfo CLI для токена ASN/geo**:
   ```
   brew install ipinfo-cli   # https://github.com/ipinfo/cli
   ipinfo login              # вставьте токен из https://ipinfo.io
   ```

5. **Сохраните код** в файл `doggi.py` и сделайте исполняемым:
   ```
   chmod +x doggi.py
   ```

6. **(Опционально) Глобальный запуск `doggi ...`**:
   ```
   mkdir -p ~/bin
   cat <<'EOF' > ~/bin/doggi
   #!/bin/bash
